import { messageSchema, type ClientMovePayload, type ClientRegisterPayload, } from '../schemas/websocket-message.schema';
...
export const handleClientRegister = (clientId: string, payload: ClientRegisterPayload): HandlerResult => {
  ...
  return {
    personal: [
      ...
      {
        type: 'CLIENT_STATE',
        payload: clientsService.getAllClients().filter(client => client.clientId !== clientId)
      }
    ],
    broadcast: [{
      type: 'CLIENT_JOIN',
